...
const ListOfAllBillingAddressCard = ({ open, onClose }) => {
    ...
    if (open) {
      API.listAllBillingAddress()
        .then((response) => {
          ...
          setBillingAddresses(response.data); // Set the response data directly
        })
        .catch((error) => {
  ...
  return (
    <>
                    ...
                    secondaryAction={
                      <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
                        <Button
                          variant="text"
                          color="primary"
                          onClick={() => handleEditClick(address)} // Pass the selected address
                        >
                          Edit
                        </Button>
                      </Box>
      ...
      {/* Passing selectedAddress as a prop to the UpdateBillingAddress component */}
      <UpdateBillingAddress
        open={isUpdateDialogOpen}
        onClose={handleCloseUpdateDialog}
        address={selectedAddress} // Passing the selected address
      />
    </>
  );
};
